<script lang="ts">
	...
	onMount(() => {
		pb.collection('notifications')
			.getFullList({ fields: 'id,text,created' })
			.then((resp) => {
...
<div class="rounded-box border-base-content/5 bg-base-100 h-96 overflow-auto border">
		...
		<tbody>
			{#each $notifications as notification}
				<tr>
					...
					<th
						>{new Date(notification.created).toLocaleString('en-GB', {
							day: '2-digit',
							month: '2-digit',
							year: 'numeric',
							hour: '2-digit',
					...
					<td
						><button
							...
							on:click={() => clearNotification(notification.id)}>Clear</button
						></td
					>
				</tr>
			{/each}
