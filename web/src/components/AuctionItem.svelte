...
<div class="card bg-base-200 shadow-xl hover:shadow-2xl transition-all duration-300 {item.winner==$user?.id?'ring-4 shadow-[0_0_20px] ring-accent shadow-accent':'border border-base-content/10'}">
  ...
  <div class="card-body p-6">
    ...
    {#if item.state === 'ongoing'}
      <button class="btn btn-primary btn-lg w-full gap-2 shadow-lg" on:click={openModal}>
  ...
  <div class="modal modal-open">
    <div class="modal-box max-w-2xl shadow-2xl">
				...
				<div class="bg-base-200 rounded-box p-4">
					<div class="flex items-center justify-between">
						...
						<div class="flex items-center gap-2">
							...
							<span class="text-3xl font-bold text-primary">{currentBid}</span>
						</div>
					</div>
				</div>
				<div class="form-control">
					<label class="label" for="bidAmount">
						<span class="label-text font-semibold text-lg">Your Bid Amount</span>
					</label>
					<input 
						type="number" 
						id="bidAmount" 
      ...
      <div class="modal-action gap-2">
        <button class="btn btn-primary btn-lg gap-2" on:click={confirmBid}>
					<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
					</svg>
					Confirm Bid
				</button>
        <button class="btn btn-lg gap-2" on:click={closeModal}>
					<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
