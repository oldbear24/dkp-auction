	...
	async function toggleFavourite(event?: Event) {
		...
		try {
			if (isFavourite && favouriteId) {
				...
				isFavourite = false;
				favouriteId = null;
				dispatch('favouriteToggled', { itemId: item.id, isFavourite: false });
			} else {
				const rec = await pb.collection('favourites').create({ auction: item.id, user: $user.id });
				showToast('Added to favourites', 'success');
				isFavourite = true;
				favouriteId = rec.id;
				dispatch('favouriteToggled', { itemId: item.id, isFavourite: true });
			}
		} catch (err: any) {
...
<div
	class="card bg-base-200 {item.winner == $user?.id
		? 'ring-accent shadow-accent shadow-[0_0_15px] ring-2'
		: 'shadow-lg'}	"
>
	<figure>
		<img src={getImage(item)} alt={item.itemName} class="h-48 w-full rounded-lg object-cover" />
	</figure>
	<div class="card-body">
		<div class="flex w-full items-start justify-between">
			<h2 class="card-title text-xl font-bold underline decoration-gray-300">{item.itemName}</h2>
			<button
				class="btn btn-ghost btn-circle"
				on:click|stopPropagation={toggleFavourite}
		...
		<p class="max-h-20 overflow-auto whitespace-pre-wrap">{item.description}</p>
		<p class="text-lg font-bold">Current Bid: {currentBid}</p>
